[![Tests](https://github.com/net7/ckanext-opendata-theme/workflows/Tests/badge.svg?branch=main)](https://github.com/net7/ckanext-opendata-theme/actions)
...
### Comandi per la gestione dei harvest sources:

 ```docker exec -it ckan-db psql -U ckan_default -d ckan_default -c "SELECT id, url, title, active, created FROM harvest_source WHERE title = 'nome_harvest';" ```

 ```docker exec -it ckan-db psql -U ckan_default -d ckan_default -c "DELETE FROM harvest_source WHERE title = 'nome_harvest';" ```
